'''
   ...
   Fall 2023
   Wenqing Zeng
   Homework 2: Turtle graphic function
''' 
...
def draw_square(myTurtle, sideLength, turtleAngle):
    ...
    Returns: turtle.Turtle, Turtle object, for application
    Ensure: turtle, pen and color status
    '''
    myTurtle.showturtle()
    myTurtle.pendown()
    myTurtle.begin_fill()
...
def ask_pos():
    ...
    Ensure: casting String input into integer so no need for another casting when return value
    '''
    ...
    x_pos = input(inquiry_x)
    y_pos = input(inquiry_y)
    x1_pos = input(inquiry_x1)
    y1_pos = input(inquiry_y1)
    return int(x_pos), int(y_pos), int(x1_pos), int(y1_pos)
