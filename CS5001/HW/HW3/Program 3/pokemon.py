...
def rps_to_string(selection):
    ...
    Function: rps_to_string()
        Converts a numerical representation of Rock-Paper-Scissors to its string equivalent
    Parameters:
        selection — int — Numerical representation of the choice
    Returns: str, Returns the string equivalent ("ROCK", "PAPER", "SCISSORS") of given selections
    '''
    if selection == 1:
        return "ROCK"
    elif selection == 2:
...
def get_player(num):
    '''
     ...
     Parameters:
         num — int — The numerical identifier for the Pokémon (ranges from 1 to 9)
     Returns: str — Returns the name of the Pokémon corresponding to the numerical identifier
...
def check_battle(computer, player):
    '''
     ...
     Returns: str, Returns "DRAW!" if it's a draw, "COMPUTER" if the computer wins, and "PLAYER" if the player wins
     '''
    if computer == player:
        return "DRAW!"
...
def game_loop(player_team, computer_team, player_win_track, computer_win_track):
    '''
    ...
    while True:
        ...
        # and the respective winner ("I" is computer and "YOU" player)
            ...
            if player_win_track > computer_win_track:
                print(
                    f"{computer_team.upper()} team: {computer_win_track}"
                    f"  {player_team.upper()} team: {player_win_track}"
                    f"\nYOU WIN!")
            else:
                print(
                    f"{computer_team.upper()} team: {computer_win_track}"
